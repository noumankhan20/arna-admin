...
export default function CMSAdminPanel() {
  ...
  return (
    <div className="min-h-screen bg-gray-50 flex">
      {/* SIDEBAR */}
      <aside
  className={`fixed lg:static inset-y-0 left-0 z-50 w-72 bg-white border-r border-gray-100 transform transition-transform duration-300 ${
    sidebarOpen ? "translate-x-0" : "-translate-x-full lg:translate-x-0"
  }`}
>
  <div className="h-full flex flex-col">
    {/* Logo / Brand with gradient header */}
    <div className="px-6 py-5 border-b border-transparent bg-gradient-to-r from-emerald-50 via-emerald-50/60 to-transparent">
      <div>
        <h1 className="text-xl font-bold text-gray-900">CMS Admin</h1>
        <p className="text-xs text-emerald-700 mt-0.5">Arna Skin Care</p>
      </div>
      <button
        onClick={() => setSidebarOpen(false)}
        className="lg:hidden text-gray-400 hover:text-gray-600 absolute right-4 top-5"
      >
        <X className="w-5 h-5" />
      </button>
    </div>
    ...
    {/* Navigation */}
    <nav className="flex-1 overflow-y-auto px-4 pt-4 pb-4 space-y-1">
      {navItems.map((item) => {
        ...
        return (
          <button
            ...
            className={`group w-full flex items-center gap-3 px-3 py-2.5 rounded-xl text-sm font-medium antialiased transition-colors ${
              isActive
