...
import { MapPin, Calendar, MoreHorizontal, ArrowRight, Package, Search } from 'lucide-react';
    ...
    return (
        <div className="bg-white rounded-[2.5rem] border border-gray-100 shadow-sm overflow-hidden">
            <div className="p-8 border-b border-gray-50 flex flex-col sm:flex-row sm:items-center justify-between gap-4">
                <div>
                    <h3 className="text-xl font-black text-gray-900 tracking-tight">Shipment Log</h3>
                                ...
                                <tr key={item.id} className="hover:bg-gray-50/80 transition-all group group/row">
                                    <td className="px-8 py-6">
                                        <div className="flex items-center gap-4">
                                            ...
                                            <div>
                                                <div className="flex items-center gap-2">
                                                    <p className="font-bold text-gray-900 tracking-tight">{item.id}</p>
                                                    <ArrowRight className="w-3 h-3 text-gray-300 group-hover/row:translate-x-1 transition-transform" />
                                                </div>
                                                <p className="text-[13px] text-gray-500 font-medium">
                                                    {item.customer} • <span className="text-emerald-600 font-bold">{item.courier}</span>
                                                </p>
                                            </div>
                                        </div>
                                    </td>
                                    <td className="px-8 py-6">
                                        <div className="flex items-center gap-2 text-[13px] text-gray-600 font-medium">
                                            <div className="p-1.5 bg-gray-50 rounded-lg group-hover/row:bg-white transition-colors">
                                    ...
                                    <td className="px-8 py-6 text-right">
                                        <button className="w-10 h-10 flex items-center justify-center hover:bg-white hover:shadow-md rounded-xl transition-all border border-transparent hover:border-gray-100 group/btn ml-auto">
                                            <MoreHorizontal className="w-5 h-5 text-gray-400 group-hover/btn:text-gray-900" />
                                        </button>
                                    </td>
                                </tr>
                            ))
                            ...
                            <tr>
                                <td colSpan="5" className="px-8 py-20 text-center">
                                    <div className="flex flex-col items-center gap-3">
                                        ...
                                        <p className="text-gray-900 font-bold tracking-tight">No shipments found</p>
            ...
            <div className="p-8 bg-gray-50/50 border-t border-gray-50 flex flex-col sm:flex-row items-center justify-between gap-4">
                <p className="text-[13px] text-gray-500 font-medium">
                    Showing <span className="text-gray-900 font-bold">{shipments.length}</span> of {totalCount} total shipments
                </p>
                <div className="flex items-center gap-2">
                    ...
                    <button className="px-6 py-3 bg-white border border-gray-100 text-gray-900 rounded-2xl text-[13px] font-bold hover:bg-emerald-600 hover:text-white hover:border-emerald-600 transition-all shadow-sm active:scale-95">Next Segment</button>
                </div>
            </div>
        </div>
    );
